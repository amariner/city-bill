...
- [~] **T4.2 Crecimiento por etapas.** En celdas zonificadas (o adyacentes a carretera
  ...
  *ANIMACIÓN DE CONSTRUCCIÓN HECHA (2026-07-05):* `world/render/construction.ts`
  (`ConstructionView`). Al llegar `cityGrew`, el edificio NO aparece de golpe: se
  despeja un solar de tierra (`constructionSite`), se levanta un ANDAMIO low-poly
  (postes + travesaños, madera `scaffold`/`scaffoldShade`) y el edificio EMERGE
...
- 2026-07-05 (sesión Opus, veta CONSTRUCCIÓN AUTÓNOMA + INTERFAZ vendible) — Objetivo:
  ...
  · **NOTA de integración (merge):** esta veta y el BANCO DE PRUEBAS `?scene=test-dev`
    (entrada siguiente) se desarrollaron en paralelo y se fusionaron. `main.ts` quedó
    con la estructura del banco (`buildRenderAndUi`), la `ControlBar` creada dentro de
    ella, y `?scene=grown` como vitrina ESTÁTICA aparte (la VIVA es `?scene=test-dev`).
