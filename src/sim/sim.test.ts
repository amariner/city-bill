...
import { deathChance, lifeYear, OLD_AGE, ADULT_AGE, RETIREMENT_AGE } from './lifecycle';
...
// el modo en su propia columna (contrato §1.3: AGENT_STRIDE, importado de protocol).
{
  const sim = new Simulation(seedWorld(), 42);
...
// --- Jubilación (ciclo 12) — graft del merge -------------------------------
// Propiedad ESTRUCTURAL (no una tirada larga con umbral, ver RESEARCH.md): un
// empleado por encima de RETIREMENT_AGE libera su puesto y se anota para narrar.
{
  const sim = new Simulation(seedFarm(11), 11) as unknown as { citizens: Map<number, { id: number; age: number; work: unknown }> };
  const employed = [...sim.citizens.values()].filter((c) => c.work);
  if (employed.length > 0) {
    for (const c of employed) c.age = RETIREMENT_AGE + 1;
    const ev = lifeYear(sim.citizens as never, createRng(11));
    const retiredIds = new Set(ev.retirements.map((r) => r.id));
    const deadIds = new Set(ev.deaths.map((d) => d.id));
    const survivors = employed.filter((c) => !deadIds.has(c.id));
    check('jubilación (ciclo 12): a >= RETIREMENT_AGE se libera el empleo', survivors.every((c) => c.work === null), `→ ${survivors.length} supervivientes`);
    check('jubilación: cada jubilado se anota para narrar (citizenRetired)', survivors.every((c) => retiredIds.has(c.id)));
  } else {
    check('jubilación: (sin empleados en la semilla — test omitido)', true);
  }
}

// --- Duelo en el snapshot (AGENT_STRIDE=8) — graft del merge -----------------
// El duelo viaja en la 8ª columna para que el render apague la ropa del doliente.
{
  const sim = new Simulation(seedFarm(5), 5) as unknown as { citizens: Map<number, { id: number; grief: number }>; snapshot: () => Float32Array };
  const first = [...sim.citizens.values()][0];
  first.grief = 0.7;
  const snap = sim.snapshot();
  check('AGENT_STRIDE incluye la columna de duelo (=8)', AGENT_STRIDE === 8);
  let found = -1;
