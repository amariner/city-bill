...
// --- Colocación y colisión de edificios -------------------------------------
{
  ...
  assert(g.placeBuilding('licensed-house', 1, 1, 30, 30, 0, 6, 'cottage'), 'placeBuilding admite capacidad y tipología por parcela');
  assert(g.get(30, 30)?.building?.housingCapacity === 6, 'la capacidad por parcela queda en el grid');
  assert(g.get(30, 30)?.building?.visualId === 'cottage', 'la tipología visual queda en el grid');
}
...
// --- Serialización de ida y vuelta ------------------------------------------
{
  ...
  g.placeBuilding('barn', 3, 2, 0, 0, 0, 5, 'cottage');
  ...
  assert(g2.get(0, 0)?.building?.visualId === 'cottage', 'tipología visual sobrevive round-trip');
